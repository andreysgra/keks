# Техническое задание к проекту «Кондитерская Кекс»
...
Приложение состоит из нескольких страниц: Каталог `/Catalog`, Избранное `/Favourites`, 404 `/404`, Вход `/logIn`, Регистрация `/SignUp`, Главная `/Index`, Страница ошибки `/ErrorPage`, Страница товара `/ProductPage`.
...
#### Список карточек товара
  ...
  В каталоге изначально отображается до шести карточек товара. Если каталог содержит больше карточек, пользователю доступна кнопка `показать ещё`. При клике на ней отрисовывается следующая порция карточек. После того, как пользователю показаны все возможные карточки, кнопка `показать ещё` скрывается, а вместо неё показывается кнопка `в начало`. При клике на ней страница прокручивается до самого верха.
...
Блок содержит информацию: наименование товара, цена, вес, рейтинг, описание товара — и кнопку добавления в избранное.
...
При заполнении текстового поля пользователю доступен счётчиков символов — при каждом введении нового символа счётчик указывает оставшееся доступное для ввода количество символов. Ограничение для поля 500 символов.
